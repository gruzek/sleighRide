...
| `android/` | The Gradle build template for the Android export. Generated by the editor, gitignored |

## Building for Android

The application ships on iPhone and Android from the same source. iOS exports through Xcode from
`exports/ios2/`; Android needs a toolchain set up once, and the versions are not a matter of taste.
They are read from `config.gradle` inside the `android_source.zip` that Godot 4.7.2 ships, and a
newer one fails rather than degrading: the Android Gradle Plugin is 8.6.1, the Gradle wrapper is
8.11.1, and neither runs on a Java Development Kit past 17.

```bash
brew install --cask temurin@17
brew install --cask android-commandlinetools
sdkmanager --sdk_root="$HOME/Library/Android/sdk" "platforms;android-36" "build-tools;36.0.0"
sdkmanager --sdk_root="$HOME/Library/Android/sdk" --licenses
```

The `--sdk_root` is not optional. Without it `sdkmanager` installs into Homebrew's own SDK
directory rather than the one the editor is pointed at, and the export then reports a missing
platform while the packages sit installed somewhere else.

Then, in the editor: **Godot → Editor Settings → Export → Android** for the Java Development Kit
path (`/usr/libexec/java_home -v 17`) and the Software Development Kit path, and **Project →
Install Android Build Template** to write `android/build/`.

A Gradle build is required rather than optional, because `addons/SharePlugin` ships its Android
half as an archive and Godot's pre-built template cannot package one. The plugin declares its own
packaging through an `EditorExportPlugin`, so there is no plugin definition file to write.

**Do not lower the minimum Software Development Kit level below 24.** The engine's Android camera
backend is built on Camera2, which requires 24, and lowering the floor removes the camera with no
error. Leaving `gradle_build/min_sdk` and `gradle_build/target_sdk` empty is what keeps the build
template's own values, which are already correct.
