...
- **The composition survives the browser.** The application is now aimed at a single-page app, where the window is whatever size the person's browser happens to be. A vignette sized against the viewport keeps the designer's composition on a screen nobody anticipated; a fixed-size image does not.
