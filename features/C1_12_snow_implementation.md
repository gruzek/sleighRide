...
- **This plan targets the Mobile renderer the project is configured for.** ~~Whether the turbulence field and the particle system survive a Compatibility export for the single-page app is a question for that work and is not settled here.~~ *(Corrected 2026-08-22: the SPA direction was reversed. There is no Compatibility/web export, so this question no longer arises — the Mobile renderer is the only target.)*
